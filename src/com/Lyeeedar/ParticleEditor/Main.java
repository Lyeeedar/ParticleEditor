...
public class Main extends JFrame {
	...
	public JPanel createRightOptions()
	{
		...
		SRCBlend.setSelectedItem(getBlendFString(renderer.effect.getEmitter(renderer.currentEmitter).blendFuncSRC));

		final JComboBox<String> DSTBlend = new JComboBox<String>(blendModes);
		DSTBlend.setSelectedItem(getBlendFString(renderer.effect.getEmitter(renderer.currentEmitter).blendFuncDST));

		final JComboBox<String> eBlend = new JComboBox<String>(new String[]{"ADD", "SUBTRACT", "REVERSE SUBTRACT"});
		eBlend.setSelectedItem(getBlendEString(renderer.effect.getEmitter(renderer.currentEmitter).blendEquation));
		...
		gc.gridx = 0;
		gc.gridy++;
		panel.add(new JLabel("Equation:"), gc);
		...
		button.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				pe.blendFuncSRC = getBlendFMode((String) SRCBlend.getSelectedItem());
				pe.blendFuncDST = getBlendFMode((String) DSTBlend.getSelectedItem());
				pe.blendEquation = getBlendEMode((String) eBlend.getSelectedItem());
	...
	public String getBlendEString(int mode)
	{
		if (mode == GL20.GL_FUNC_ADD) return "ADD";
		else if (mode == GL20.GL_FUNC_SUBTRACT) return "SUBTRACT";
		else if (mode == GL20.GL_FUNC_REVERSE_SUBTRACT) return "REVERSE SUBTRACT";
		else return null;
	}

	public int getBlendEMode(String mode)
	{
		if (mode.equals("ADD")) return GL20.GL_FUNC_ADD;
		else if (mode.equals("SUBTRACT")) return GL20.GL_FUNC_SUBTRACT;
		else if (mode.equals("REVERSE SUBTRACT")) return GL20.GL_FUNC_REVERSE_SUBTRACT;
		else return 0;
	}

	public String getBlendFString(int mode)
	{
	...
	public int getBlendFMode(String mode)
	{
	...
	class Renderer implements ApplicationListener
	{
		...
		{
			ParticleEmitter orb = new ParticleEmitter(2, 10, GL20.GL_SRC_ALPHA, GL20.GL_ONE, GL20.GL_FUNC_ADD, "data/atlases/orb.atlas", "blank");
		...
		public void create()
		{
			...
			apply.addActionListener(new ActionListener(){
				...
				public void actionPerformed(ActionEvent e) {
					...
					settings.pot = false;
					TexturePacker2 packer = new TexturePacker2(new File(""), settings);
